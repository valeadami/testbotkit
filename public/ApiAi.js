...
var StreamClient = (function () {
    ...
    StreamClient.prototype.init = function () {
        var _this = this;
        this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_WAITING_MICROPHONE, "Waiting for approval to access your microphone ...");
        try {
            window.AudioContext = window.AudioContext || webkitAudioContext;
            ...
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_CLIENT, "Error initializing Web Audio browser: " + JSON.stringify(e));
        }
        if (navigator.getUserMedia) {
            navigator.getUserMedia({ audio: true }, this.startUserMedia.bind(this, this.onInit), function (e) {
                _this.onError(Interfaces_1.IStreamClient.ERROR.ERR_CLIENT, "No live audio input in this browser: " + JSON.stringify(e));
            });
        }
        else {
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_CLIENT, "No user media support");
        }
    };
    StreamClient.prototype.startListening = function () {
        var _this = this;
        if (!this.recorder) {
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_AUDIO, "Recorder undefined");
            return;
        }
        if (!this.ws) {
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_AUDIO, "No web socket connection");
            return;
        }
        if (this.ws.readyState === WebSocket.CLOSED) {
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_NETWORK, "WebSocket is in 'closed' state");
            return;
        }
        var isUseVad = function (endOfSpeechCallback) {
        ...
        isUseVad(function () { return _this.stopListening(); });
        this.ws
    ...
    StreamClient.prototype.stopListening = function () {
        ...
        // Stop recording
        recorder.stop();
        this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_STOP, "Stopped recording");
    ...
    StreamClient.prototype.startUserMedia = function (onInit, stream) {
        ...
        this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_MEDIA_STREAM_CREATED, "Media stream created");
        ...
        this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_INIT_RECORDER, "Recorder initialized");
        if (onInit) {
            onInit();
        }
    };
    StreamClient.prototype.openWebSocket = function () {
        ...
        try {
            ...
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_CLIENT, "No web socket support in this browser!");
        }
    };
    ;
    StreamClient.prototype.createWebSocket = function () {
        ...
        ws.onmessage = function (e) {
            var data = e.data;
            _this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_WEB_SOCKET, data);
            if (data instanceof Object && !(data instanceof Blob)) {
                _this.onError(Interfaces_1.IStreamClient.ERROR.ERR_SERVER, "WebSocket: onEvent: got Object that is not a Blob");
            }
            else if (data instanceof Blob) {
                _this.onError(Interfaces_1.IStreamClient.ERROR.ERR_SERVER, "WebSocket: got Blob");
            }
            else {
        ...
        ws.onopen = function (e) {
            ...
            _this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_WEB_SOCKET_OPEN, e);
        };
        ...
        ws.onclose = function (e) {
            ...
            _this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_WEB_SOCKET_CLOSE, e.code + "/" + e.reason + "/" + e.wasClean);
        };
        ws.onerror = function (e) {
            _this.onError(Interfaces_1.IStreamClient.ERROR.ERR_NETWORK, JSON.stringify(e.data));
        };
        return ws;
    };
    StreamClient.prototype.socketSend = function (item) {
        if (!this.ws) {
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_CLIENT, "No web socket connection: failed to send: " + item);
            return;
        }
        ...
        if (state !== 1) {
            ...
            this.onError(Interfaces_1.IStreamClient.ERROR.ERR_NETWORK, errorMsg);
        }
        // If item isn't an audio blob it's the EOS tag or json data (string)
        if (!(item instanceof Blob)) {
            this.ws.send(item);
            this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_SEND_EOS_OR_JSON, "Send string: " + item);
        }
        else if (item.size > 0) {
            this.ws.send(item);
            this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_SEND, "Send: blob: " + item.type + ", " + item.size);
        }
        else {
            this.onEvent(Interfaces_1.IStreamClient.EVENT.MSG_SEND_EMPTY, "Send: blob: " + item.type + ", EMPTY");
        }
    };
    return StreamClient;
}());
